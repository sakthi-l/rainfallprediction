    ...
    # Flood/Drought Prediction
        ...
        if st.button("🔍 Predict Condition", type="primary"):
            ...
            if error:
                ...
                if max_prob > 80:
                    st.success("**Very High Confidence**: Model is very certain about the prediction.")
                elif max_prob > 60:
                    st.info("**High Confidence**: Model shows strong certainty in prediction.")
                elif max_prob > 40:
                    st.warning("**Moderate Confidence**: Conditions are borderline. Monitor closely.")
                else:
                    st.error("**Low Confidence**: Uncertain conditions. Consider multiple scenarios.")
                
                # Recommendations
                with st.expander("📋 Recommended Actions"):
                    if pred_label == "Flood":
                        st.markdown("""
                        - **Immediate**: Check drainage systems and flood defenses
                        ...
                        - **Agriculture**: Prepare drought-resistant crop strategies
                        - **Planning**: Water rationing and distribution plans
                        - **Monitoring**: Groundwater levels and reservoir status
                        """)
                    else:
                        st.markdown("""
                        - **Routine Monitoring**: Continue regular weather tracking
                        - **Preparedness**: Maintain standard emergency protocols
                        - **Agriculture**: Follow normal cultivation practices
                        - **Infrastructure**: Regular maintenance of water systems
                        """)

    # Analytics
    else:
        st.header("📈 Regional Analytics & Model Performance")
        
        tab1, tab2, tab3 = st.tabs([
            "📍 Rainfall Regions", 
            "🌊 Flood/Drought Regions",
            "📊 Model Performance"
        ])
        ...
        with tab1:
            ...
            if df is not None:
                ...
                # Region list with statistics
                region_stats = []
                for region in sorted(df['SUBDIVISION'].unique()):
                    region_data = df[df['SUBDIVISION'] == region]
                    region_stats.append({
                        'Region': region,
                        'Years Available': f"{region_data['YEAR'].min()}-{region_data['YEAR'].max()}",
                        'Avg Rainfall': f"{region_data['ANNUAL'].mean():.2f}",
                        'Variability': f"{region_data['ANNUAL'].std():.2f}"
                    })
                ...
                # Distribution plot
                st.subheader("📊 Rainfall Distribution by Region")
                fig = px.box(
                    df, 
                    x='SUBDIVISION', 
                    y='ANNUAL',
                    title='Annual Rainfall Distribution',
                    labels={'ANNUAL': 'Rainfall (mm)', 'SUBDIVISION': 'Region'}
                )
        ...
        with tab2:
            clf, le, df, feature_cols, months = load_and_train_flood_model()
            if df is not None:
                st.subheader("Region Statistics - Flood/Drought Analysis")
                
                region_stats = df.groupby('SUBDIVISION').agg({
                    'YEAR': ['min', 'max', 'count'],
                    'Total': ['mean', 'min', 'max', 'std'],
                ...
                region_stats.columns = ['Year_Min', 'Year_Max', 'Records', 
                                       ...
                                       'Std_Rainfall', 'Label_Distribution']
                
                st.dataframe(region_stats, use_container_width=True)
                
                # Class distribution
                st.subheader("📊 Historical Event Distribution")
                ...
                fig.update_layout(title="Overall Distribution of Events", height=400)
                st.plotly_chart(fig, use_container_width=True)
        ...
        with tab3:
            ...
            with col1:
                st.markdown("**Rainfall Forecasting Model**")
                st.markdown("""
                - **Architecture**: Multi-model ensemble
                - **Models**: XGBoost + Random Forest + Gradient Boosting + Historical baseline
                - **Features**: 120+ engineered features
                - **Validation**: Time series cross-validation
                - **Expected Accuracy**: 90-95% (region-dependent)
                """)
                
                st.info("""
                **Key Improvements:**
                - Enhanced feature engineering (lags, seasonality, interactions)
                - Region-specific ensemble weighting
                - Boundary constraint validation
                - Multiple model combination for robustness
                """)
            
            with col2:
                st.markdown("**Flood/Drought Classification Model**")
                st.markdown("""
                - **Algorithm**: XGBoost Classifier (800 estimators)
                - **Features**: 50+ rainfall and temporal features
                - **Balancing**: SMOTE with stratified sampling
                - **Classes**: Flood, Normal, Drought
                ...
                **Key Improvements:**
                - Seasonal pattern analysis
                - Multi-year temporal dependencies
                - Enhanced SPI calculation with regional normalization
                - Improved class balancing techniques
                """)
            
            st.markdown("---")
            st.subheader("🔍 Error Analysis Tips")
            st.markdown("""
            **To minimize prediction errors:**
            
            1. **Data Quality**: Ensure input data is complete and accurate
            2. **Region Selection**: Choose exact region names from the dataset
            3. **Temporal Consistency**: Use sequential years for better temporal feature extraction
            4. **Manual Input**: When entering monthly data, ensure realistic values based on regional patterns
            5. **Validation**: Cross-check predictions with historical patterns and trends
            
            **Known Limitations:**
            - Extreme weather events may not be fully captured
            - Limited accuracy for regions with high climate variability
            - Future predictions (>5 years) have higher uncertainty
            - Manual inputs should reflect realistic seasonal patterns
            """)
